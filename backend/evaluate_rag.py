...
DB 문서 기반 골드셋으로 4가지 지표 전부 측정합니다.

평가 지표:
  - Faithfulness: 답변이 검색된 청크에 근거하는가
  - AnswerRelevancy: 답변이 질문과 관련 있는가
  - ContextPrecision: 검색된 컨텍스트가 관련 있는가
  - ContextRecall: 관련 내용을 빠짐없이 검색했는가
"""
import os
import sys
import types
...
from config import GEMINI_API_KEY, GOOGLE_APPLICATION_CREDENTIALS, GOOGLE_CLOUD_PROJECT
...
from services.rag import classify_and_rewrite, search_rag, ChatState
from services.rag import POLICY_SYSTEM, PARENTING_SYSTEM, FIRST_AID_SYSTEM
...
GOLD_QUERIES = {
    ...
    "first_aid": [
        (
            "아이가 끓는 물에 데었을 때 초기 응급처치는 어떻게 해야 하나요?",
        ...
        (
            "아이의 귀에 구슬이나 콩이 들어갔을 때 어떻게 해야 하나요?",
...
SYSTEM_MAP = {
    ...
    "parenting": PARENTING_SYSTEM,
    "first_aid": FIRST_AID_SYSTEM,
}

# ── 파이프라인 ────────────────────────────────────────────────────────────────

def run_pipeline(query: str, mode: str) -> tuple[str, list[str]]:
    initial: ChatState = {
        "query": query,
        "search_query": query,
        "history": [],
    ...
    original_max = rag_module.MAX_DISTANCE
    rag_module.MAX_DISTANCE = 0.75
    state = classify_and_rewrite(initial)
    state = search_rag(state)
    rag_module.MAX_DISTANCE = original_max

    if not state["context"]:
        return "", []

    contexts = [c.strip() for c in state["context"].split("---") if c.strip()]

    prompt = f"참고 자료:\n{state['context']}\n\n질문: {query}"
    response = _vertex_client.models.generate_content(
        model="gemini-2.5-flash",
        contents=prompt,
        config=genai_types.GenerateContentConfig(system_instruction=SYSTEM_MAP[mode]),
    )
...
def run_evaluation():
    print("\n" + "="*65)
    print("  RAGAS 4가지 지표 평가 (DB 기반 골드셋)")
    print("="*65)

    client = _vertex_client
    ragas_llm = llm_factory(model="gemini-2.5-flash", provider="google", client=client)
    ...
    for mode, queries in GOLD_QUERIES.items():
        print(f"\n[{mode.upper()} 모드] 평가 중... ({len(queries)}개 쿼리)")
        samples = []
        valid_queries = []

        for question, ground_truth in queries:
            print(f"  → {question[:45]}...")
            answer, contexts = run_pipeline(question, mode)
            if not contexts:
        ...
        print(f"\n  {'쿼리':<42} {'Faith':>7} {'Rel':>7} {'Prec':>7} {'Rec':>7}")
        print("  " + "-"*72)
        for i, (_, row) in enumerate(df.iterrows()):
            q = valid_queries[i][:40]
            print(f"  {q:<42} {row['faithfulness']:>7.3f} {row['answer_relevancy']:>7.3f} {row['context_precision']:>7.3f} {row['context_recall']:>7.3f}")
        print("  " + "-"*72)
        print(f"  {'평균':<42} {avg_f:>7.3f} {avg_ar:>7.3f} {avg_cp:>7.3f} {avg_cr:>7.3f}")

        all_results.append({"mode": mode, "faithfulness": avg_f, "answer_relevancy": avg_ar,
                            "context_precision": avg_cp, "context_recall": avg_cr})

    print("\n" + "="*65)
    print("  모드별 요약")
    print("="*65)
    print(f"  {'모드':<12} {'Faithfulness':>13} {'Ans.Rel':>9} {'Ctx.Prec':>10} {'Ctx.Rec':>9}")
    print("  " + "-"*55)
    for r in all_results:
        print(f"  {r['mode']:<12} {r['faithfulness']:>13.3f} {r['answer_relevancy']:>9.3f} {r['context_precision']:>10.3f} {r['context_recall']:>9.3f}")

    if all_results:
        tf  = sum(r["faithfulness"]       for r in all_results) / len(all_results)
        tar = sum(r["answer_relevancy"]   for r in all_results) / len(all_results)
        tcp = sum(r["context_precision"]  for r in all_results) / len(all_results)
        tcr = sum(r["context_recall"]     for r in all_results) / len(all_results)
        print("  " + "-"*55)
        print(f"  {'전체 평균':<12} {tf:>13.3f} {tar:>9.3f} {tcp:>10.3f} {tcr:>9.3f}")
    print("="*65 + "\n")
